...
Issue #72 adds the first local project browser: create, open, rename, duplicate, and confirmed delete. The old single `active` workspace migrates into stable project records plus an active-project index. Duplicate projects reuse hashed media, and deletion checks every remaining project plus the Soundtrack before reclaiming an audio Blob. Portable archive backup/restore, explicit storage-usage reporting, and undo/redo remain focused follow-up slices.
