import { arachnidRig, blobRig, expressiveRig, giantRig, humanoidRig } from '@/game/sprites/rigs';
import type { EnemyDef } from '@/game/types';
import { palette } from './authoring';
import { REACTION_PRESETS } from './reactivity';

/**
 * Everything that wants the player gone. `behavior` selects the movement
 * routine in the simulation; the rest is tuning.
 */
export const ENEMIES: EnemyDef[] = [
  {
    id: 'watchlight',
    name: 'Watchlight',
    family: 'Street',
    behavior: 'sentry',
    hp: 16,
    speed: 30,
    damage: 3,
    radius: 9,
    xp: 3,
    mass: 1,
    faction: 'The Watch',
    traits: { coneDetect: { range: 260, halfAngleDeg: 24, sweepSpeed: 0.7 } },
    palette: palette({ ink: '#0a0a12', body: '#78350f', bodyDark: '#451a03', accent: '#f59e0b', glow: '#fde68a' }),
    rig: humanoidRig({ height: 16, width: 9, cap: true }),
    lore: 'A parking-lot floodlight that learned to walk. Its beam finds what wants to stay hidden.',
  },
  {
    id: 'floodline',
    name: 'Floodline',
    family: 'Street',
    behavior: 'sentry',
    hp: 26,
    speed: 0,
    damage: 4,
    radius: 11,
    xp: 4,
    mass: 2.4,
    faction: 'The Watch',
    // Zero speed makes the sentry case's own approach movement a no-op, so
    // this one plants itself and only sweeps -- a fixed watchtower instead
    // of Watchlight's slow patrol. See "sentry" in engine/world.ts.
    traits: { coneDetect: { range: 330, halfAngleDeg: 16, sweepSpeed: 0.4 } },
    palette: palette({ ink: '#0a0a12', body: '#1e293b', bodyDark: '#0f172a', accent: '#f59e0b', glow: '#fde68a' }),
    rig: giantRig(20),
    lore: 'Bolted to a rooftop, its lens never blinks. Longest sightline of anything in 616.',
  },
  {
    id: 'clampjaw',
    name: 'Clampjaw',
    family: 'Street',
    behavior: 'tracker',
    hp: 22,
    speed: 40,
    damage: 5,
    radius: 10,
    xp: 5,
    mass: 1.2,
    faction: 'Lockstep',
    // Fast, tight, quick to close -- the "hit and back off" tracker. Its
    // cone always faces the player's real position (see 'tracker' in
    // engine/world.ts) and resets to open the moment you step out of range.
    traits: { lockCone: { range: 220, startHalfAngleDeg: 30, minHalfAngleDeg: 1.5, closeMs: 2200, explodeDamagePct: 0.22, resetMs: 1200 } },
    palette: palette({ ink: '#0a0a12', body: '#7f1d1d', bodyDark: '#450a0a', accent: '#ef4444', glow: '#fca5a5' }),
    rig: arachnidRig({ height: 12, span: 14 }),
    lore: 'Its pincers read your heat signature and close on it like a trap.',
  },
  {
    id: 'deadlock',
    name: 'Deadlock',
    family: 'Street',
    behavior: 'tracker',
    hp: 40,
    speed: 22,
    damage: 8,
    radius: 13,
    xp: 7,
    mass: 2.6,
    faction: 'Lockstep',
    // Slower to close but hits much harder -- gives more warning, punishes
    // more for ignoring it.
    traits: { lockCone: { range: 260, startHalfAngleDeg: 26, minHalfAngleDeg: 1.5, closeMs: 3600, explodeDamagePct: 0.32, resetMs: 2500 } },
    palette: palette({ ink: '#0a0a12', body: '#78350f', bodyDark: '#431407', accent: '#ff2d55', glow: '#fda4af' }),
    rig: giantRig(19),
    lore: 'Old 616 ordinance, still armed. Give it a straight look and it remembers your outline.',
  },
  {
    id: 'lure-lamp',
    name: 'Lure Lamp',
    family: 'Signal',
    behavior: 'beacon',
    hp: 24,
    speed: 26,
    damage: 3,
    radius: 9,
    xp: 5,
    mass: 1,
    faction: 'Prism Choir',
    // General version: pink beam, drags the player toward it. See 'beacon' in engine/world.ts.
    traits: { colorCone: { range: 240, halfAngleDeg: 20, sweepSpeed: 0.55, kinds: ['pull'], pullForce: 60 } },
    palette: palette({ ink: '#0a0a12', body: '#831843', bodyDark: '#500724', accent: '#f472b6', glow: '#fbcfe8' }),
    rig: humanoidRig({ height: 15, width: 9, hood: true }),
    lore: 'A storefront sign with nothing left to sell. It just wants you closer.',
  },
  {
    id: 'drag-net',
    name: 'Drag Net',
    family: 'Signal',
    behavior: 'beacon',
    hp: 24,
    speed: 26,
    damage: 3,
    radius: 9,
    xp: 5,
    mass: 1,
    faction: 'Prism Choir',
    // General version: blue beam, cuts move speed while you stand in it.
    traits: { colorCone: { range: 260, halfAngleDeg: 20, sweepSpeed: 0.55, kinds: ['slow'], slowPct: 0.4 } },
    palette: palette({ ink: '#0a0a12', body: '#075985', bodyDark: '#0c4a6e', accent: '#38bdf8', glow: '#bae6fd' }),
    rig: humanoidRig({ height: 15, width: 9, hood: true }),
    lore: 'Broadcasts on a frequency that makes your own feet feel like a suggestion.',
  },
  {
    id: 'cinder-eye',
    name: 'Cinder Eye',
    family: 'Signal',
    behavior: 'beacon',
    hp: 20,
    speed: 24,
    damage: 3,
    radius: 8,
    xp: 6,
    mass: 0.9,
    faction: 'Prism Choir',
    // Elemental version: orange beam, ticks burn damage while you stand in it.
    traits: { colorCone: { range: 230, halfAngleDeg: 18, sweepSpeed: 0.6, kinds: ['burn'], tickDamagePerSec: 7 } },
    palette: palette({ ink: '#180d08', body: '#7c2d12', bodyDark: '#431407', accent: '#fb923c', glow: '#fed7aa' }),
    rig: blobRig({ height: 12, width: 10, spikes: true }),
    lore: 'Never blinks. Never cools down either.',
  },
  {
    id: 'frost-eye',
    name: 'Frost Eye',
    family: 'Signal',
    behavior: 'beacon',
    hp: 20,
    speed: 24,
    damage: 3,
    radius: 8,
    xp: 6,
    mass: 0.9,
    faction: 'Prism Choir',
    // Elemental version: pale blue beam, a harder slow than Drag Net's.
    traits: { colorCone: { range: 230, halfAngleDeg: 18, sweepSpeed: 0.6, kinds: ['chill'], slowPct: 0.4 } },
    palette: palette({ ink: '#07111b', body: '#1e3a5f', bodyDark: '#0c1f33', accent: '#93c5fd', glow: '#dbeafe' }),
    rig: blobRig({ height: 12, width: 10, tendrils: true }),
    lore: 'Fogs the block glass-cold around it. Everything on the other side moves slower.',
  },
  {
    id: 'volt-eye',
    name: 'Volt Eye',
    family: 'Signal',
    behavior: 'beacon',
    hp: 20,
    speed: 24,
    damage: 3,
    radius: 8,
    xp: 6,
    mass: 0.9,
    faction: 'Prism Choir',
    // Elemental version: violet beam, ticks shock damage while you stand in it.
    traits: { colorCone: { range: 230, halfAngleDeg: 18, sweepSpeed: 0.6, kinds: ['shock'], tickDamagePerSec: 7 } },
    palette: palette({ ink: '#0f0a1a', body: '#4c1d95', bodyDark: '#2e1065', accent: '#a78bfa', glow: '#ddd6fe' }),
    rig: blobRig({ height: 12, width: 10, spikes: true }),
    lore: 'Skips off every downed line on the block like it is still live.',
  },
  {
    id: 'prism-warden',
    name: 'Prism Warden',
    family: 'Signal',
    behavior: 'beacon',
    hp: 90,
    speed: 20,
    damage: 6,
    radius: 13,
    xp: 16,
    mass: 2,
    sizeClass: 'elite',
    faction: 'Prism Choir',
    // Higher tier: uses every version, flickering the beam's color/effect
    // through all five every 900ms instead of committing to one.
    traits: {
      colorCone: {
        range: 260,
        halfAngleDeg: 22,
        sweepSpeed: 0.5,
        kinds: ['pull', 'slow', 'chill', 'burn', 'shock'],
        flickerMs: 900,
        pullForce: 60,
        slowPct: 0.4,
        tickDamagePerSec: 7,
      },
    },
    palette: palette({ ink: '#0a0a12', body: '#312e81', bodyDark: '#1e1b4b', accent: '#c4b5fd', glow: '#ede9fe' }),
    rig: giantRig(24),
    lore: 'Every dead sign on the block reflected in one face at once.',
  },
  {
    id: 'kaleidoscope',
    name: 'Kaleidoscope',
    family: 'Boss',
    behavior: 'beacon',
    hp: 620,
    speed: 18,
    damage: 10,
    radius: 24,
    xp: 90,
    mass: 6,
    sizeClass: 'giant',
    // The boss tier: same all-versions flicker as Prism Warden, but faster,
    // wider, and every effect hits harder.
    traits: {
      colorCone: {
        range: 320,
        halfAngleDeg: 26,
        sweepSpeed: 0.45,
        kinds: ['pull', 'slow', 'chill', 'burn', 'shock'],
        flickerMs: 600,
        pullForce: 95,
        slowPct: 0.5,
        tickDamagePerSec: 11,
      },
    },
    palette: palette({ ink: '#05030a', body: '#4c1d95', bodyDark: '#1e1b4b', accent: '#f472b6', accentBright: '#ffffff', glow: '#c4b5fd' }),
    rig: giantRig(34),
    lore: 'Every sign in 616 that ever went dark, still trying to tell you something.',
  },
  {
    id: 'nightcrawler',
    react: REACTION_PRESETS.beatTwitch,
    name: 'Nightcrawler',
    family: 'Street',
    behavior: 'chase',
    hp: 20,
    speed: 48,
    damage: 6,
    radius: 9,
    xp: 3,
    mass: 1,
    palette: {
      ink: '#0a0a12',
      body: '#3b2f57',
      bodyDark: '#221b33',
      accent: '#8b5cf6',
      accentBright: '#e0b3ff',
      skin: '#4a3a68',
      glow: '#a78bfa',
    },
    rig: humanoidRig({ height: 17, width: 9, hood: true }),
    lore: 'Was somebody who used to close down the bar. Now it just closes in.',
  },
  {
    id: 'neon-leech',
    react: REACTION_PRESETS.beatTwitch,
    name: 'Neon Leech',
    family: 'Street',
    behavior: 'drifter',
    hp: 13,
    speed: 66,
    damage: 4,
    radius: 7,
    xp: 2,
    mass: 0.6,
    palette: {
      ink: '#04121a',
      body: '#0e7490',
      bodyDark: '#083344',
      accent: '#22d3ee',
      accentBright: '#cffafe',
      skin: '#0e7490',
      glow: '#22d3ee',
    },
    rig: blobRig({ height: 12, width: 9, tendrils: true }),
    lore: 'Feeds on the buzz of sign transformers. Sticks to whoever glows brightest.',
  },
  {
    id: 'ash-wisp',
    name: 'Ash Wisp',
    family: 'Cinder',
    behavior: 'drifter',
    hp: 11,
    speed: 84,
    damage: 5,
    radius: 7,
    xp: 3,
    mass: 0.4,
    palette: {
      ink: '#180d08',
      body: '#7c2d12',
      bodyDark: '#3b1608',
      accent: '#fb923c',
      accentBright: '#fed7aa',
      skin: '#7c2d12',
      glow: '#fb923c',
    },
    rig: blobRig({ height: 11, width: 8, spikes: true }),
    lore: 'What is left over when a fire on the east side never fully went out.',
  },
  {
    id: 'corner-cutter',
    name: 'Corner Cutter',
    family: 'Street',
    behavior: 'flanker',
    hp: 22,
    speed: 58,
    damage: 7,
    radius: 8,
    xp: 5,
    mass: 0.8,
    palette: {
      ink: '#111827',
      body: '#334155',
      bodyDark: '#172033',
      accent: '#facc15',
      accentBright: '#fef9c3',
      skin: '#475569',
      glow: '#facc15',
    },
    rig: humanoidRig({ height: 17, width: 9, hood: true }),
    lore: 'Never takes the block straight. Always appears where the streetlight does not.',
  },
  {
    id: 'bass-bruiser',
    react: REACTION_PRESETS.bassBulge,
    name: 'Bass Bruiser',
    family: 'Heavy',
    behavior: 'shockwave',
    hp: 68,
    speed: 30,
    damage: 13,
    radius: 13,
    xp: 12,
    mass: 2.8,
    palette: {
      ink: '#120b16',
      body: '#581c87',
      bodyDark: '#2e1065',
      accent: '#e879f9',
      accentBright: '#fae8ff',
      skin: '#7e22ce',
      glow: '#e879f9',
    },
    rig: humanoidRig({ height: 24, width: 14 }),
    lore: 'The bass hits first. The pavement answers second.',
  },
  {
    id: 'lightless-prowler',
    name: 'Lightless Prowler',
    family: 'Shadow',
    behavior: 'prowler',
    hp: 30,
    speed: 92,
    damage: 10,
    radius: 9,
    xp: 9,
    mass: 0.7,
    palette: {
      ink: '#020617',
      body: '#111827',
      bodyDark: '#030712',
      accent: '#64748b',
      accentBright: '#e2e8f0',
      skin: '#1e293b',
      glow: '#94a3b8',
    },
    rig: blobRig({ height: 14, width: 10, spikes: true }),
    lore: 'It does not hide in the dark. It makes the dark move around it.',
  },
  {
    id: 'bridge-lookout',
    name: 'Bridge Lookout',
    family: 'Rooftop',
    behavior: 'lookout',
    hp: 38,
    speed: 40,
    damage: 6,
    radius: 9,
    xp: 10,
    mass: 1.1,
    palette: {
      ink: '#07111b',
      body: '#155e75',
      bodyDark: '#083344',
      accent: '#67e8f9',
      accentBright: '#ecfeff',
      skin: '#0e7490',
      glow: '#22d3ee',
    },
    rig: humanoidRig({ height: 19, width: 10 }),
    lore: 'Keeps the bridge toll in teeth and tracer fire.',
    ranged: { cooldownMs: 1700, projectileSpeed: 185, damage: 12 },
  },
  {
    id: 'river-wraith',
    name: 'River Wraith',
    family: 'River',
    behavior: 'current',
    hp: 44,
    speed: 52,
    damage: 9,
    radius: 10,
    xp: 11,
    mass: 0.9,
    palette: {
      ink: '#03131a',
      body: '#115e59',
      bodyDark: '#042f2e',
      accent: '#2dd4bf',
      accentBright: '#ccfbf1',
      skin: '#0f766e',
      glow: '#5eead4',
    },
    rig: blobRig({ height: 17, width: 11, tendrils: true }),
    lore: 'Pulled out of the Grand by a floodwall light that stayed on too long.',
  },
  {
    id: 'bloodhound',
    react: REACTION_PRESETS.downbeatLunge,
    name: 'Bloodhound',
    family: 'Pack',
    behavior: 'charger',
    hp: 34,
    speed: 74,
    damage: 11,
    radius: 10,
    xp: 6,
    mass: 1.2,
    palette: {
      ink: '#170406',
      body: '#9f1239',
      bodyDark: '#4c0519',
      accent: '#fb7185',
      accentBright: '#ffe4e6',
      skin: '#9f1239',
      glow: '#fb7185',
    },
    rig: humanoidRig({ height: 16, width: 10 }),
    lore: 'Runs in straight lines and commits to every one of them.',
  },
  {
    id: 'crypt-spitter',
    name: 'Crypt Spitter',
    family: 'Coven',
    behavior: 'spitter',
    hp: 28,
    speed: 34,
    damage: 5,
    radius: 9,
    xp: 7,
    mass: 1.4,
    palette: {
      ink: '#0b1408',
      body: '#3f6212',
      bodyDark: '#1a2e05',
      accent: '#a3e635',
      accentBright: '#ecfccb',
      skin: '#3f6212',
      glow: '#a3e635',
    },
    rig: humanoidRig({ height: 18, width: 10, hood: true }),
    lore: 'Keeps its distance and throws something you should not catch.',
    ranged: { cooldownMs: 2200, projectileSpeed: 150, damage: 9 },
  },
  {
    id: 'belfry-bat',
    react: REACTION_PRESETS.beatTwitch,
    name: 'Belfry Bat',
    family: 'Coven',
    behavior: 'drifter',
    hp: 9,
    speed: 112,
    damage: 3,
    radius: 6,
    xp: 2,
    mass: 0.3,
    palette: {
      ink: '#0b0714',
      body: '#4c1d95',
      bodyDark: '#2e1065',
      accent: '#c4b5fd',
      accentBright: '#f5f3ff',
      skin: '#4c1d95',
      glow: '#c4b5fd',
    },
    rig: blobRig({ height: 9, width: 11, spikes: true }),
    sizeClass: 'mini',
    lore: 'Nests in the old bell tower. Comes down in sheets when the bell is rung.',
  },
  {
    id: 'crypt-bouncer',
    react: REACTION_PRESETS.downbeatLunge,
    name: 'Crypt Bouncer',
    family: 'Elite',
    behavior: 'charger',
    hp: 110,
    speed: 42,
    damage: 17,
    radius: 15,
    xp: 18,
    mass: 3.2,
    sizeClass: 'elite',
    palette: {
      ink: '#0c0a09',
      body: '#44403c',
      bodyDark: '#1c1917',
      accent: '#f59e0b',
      accentBright: '#fef3c7',
      skin: '#57534e',
      glow: '#f59e0b',
    },
    rig: humanoidRig({ height: 26, width: 15 }),
    lore: 'Still checking IDs at a door that burned down years ago.',
  },
  {
    id: 'the-sire',
    react: REACTION_PRESETS.bassBulge,
    name: 'The Sire',
    family: 'Boss',
    behavior: 'chase',
    hp: 950,
    speed: 40,
    damage: 24,
    radius: 22,
    xp: 140,
    mass: 8,
    palette: {
      ink: '#0a0208',
      body: '#7f1d1d',
      bodyDark: '#3b0a0a',
      accent: '#ef4444',
      accentBright: '#fecaca',
      skin: '#450a0a',
      glow: '#ef4444',
    },
    rig: humanoidRig({ height: 34, width: 18, hood: true, halo: true }),
    sizeClass: 'giant',
    lore: 'The one who turned the rest. Owns half the block on paper.',
  },
  {
    id: 'spiral-moth',
    name: 'Spiral Moth',
    family: 'Shadow',
    behavior: 'flanker',
    hp: 18,
    speed: 96,
    damage: 7,
    radius: 8,
    xp: 4,
    mass: 0.5,
    palette: { ink: '#07050f', body: '#17132d', bodyDark: '#090718', accent: '#a78bfa', accentBright: '#f5f3ff', skin: '#312e81', glow: '#c4b5fd' },
    rig: expressiveRig('spiral', 15),
    faction: 'Afterimage Choir',
    role: 'flanker',
    traits: { teleportMs: 4200, ghostMs: 520 },
    lore: 'A bad thought with wings. Its spiral eyes always appear at the edge of the screen first.',
  },
  {
    id: 'neon-comet',
    react: REACTION_PRESETS.beatTwitch,
    name: 'Neon Comet',
    family: 'Cosmic',
    behavior: 'drifter',
    hp: 26,
    speed: 82,
    damage: 8,
    radius: 9,
    xp: 6,
    mass: 0.7,
    palette: { ink: '#06131d', body: '#12304a', bodyDark: '#071827', accent: '#22d3ee', accentBright: '#cffafe', skin: '#164e63', glow: '#67e8f9' },
    rig: expressiveRig('astral', 16),
    faction: 'Afterimage Choir',
    role: 'disruptor',
    traits: { ghostMs: 680, burstSpeed: 1.8 },
    lore: 'A piece of sky that missed the river and learned to chase warm bodies.',
  },
  {
    id: 'smoke-horn',
    name: 'Smoke Horn',
    family: 'Elite',
    behavior: 'charger',
    hp: 138,
    speed: 39,
    damage: 20,
    radius: 16,
    xp: 24,
    mass: 3.8,
    palette: { ink: '#18060d', body: '#7f1d4d', bodyDark: '#2b0a1d', accent: '#f43f5e', accentBright: '#fecdd3', skin: '#9f1239', glow: '#fb7185' },
    rig: expressiveRig('flame', 28),
    faction: 'Cinder Procession',
    role: 'anchor',
    sizeClass: 'elite',
    traits: { shiftMs: 2600, shiftScale: 1.35, burstSpeed: 1.6 },
    lore: 'An armored argument that exhales magenta smoke before it charges.',
  },
  {
    id: 'current-stag',
    name: 'Current Stag',
    family: 'River',
    behavior: 'current',
    hp: 52,
    speed: 64,
    damage: 10,
    radius: 11,
    xp: 13,
    mass: 0.8,
    palette: { ink: '#03131a', body: '#115e59', bodyDark: '#042f2e', accent: '#2dd4bf', accentBright: '#ccfbf1', skin: '#164e63', glow: '#67e8f9' },
    rig: expressiveRig('river', 20),
    faction: 'River Antler Court',
    role: 'carrier',
    traits: { teleportMs: 5200, shiftMs: 3400, shiftScale: 1.25 },
    lore: 'Branch-antlered water that runs sideways through the street grid.',
  },
  {
    id: 'ring-scribe',
    name: 'Ring Scribe',
    family: 'Astral',
    behavior: 'spitter',
    hp: 42,
    speed: 38,
    damage: 7,
    radius: 10,
    xp: 12,
    mass: 1.1,
    palette: { ink: '#140d1c', body: '#7c5528', bodyDark: '#2e1b35', accent: '#c4b5fd', accentBright: '#fff1b8', skin: '#d6a85f', glow: '#facc15' },
    rig: expressiveRig('astral', 18),
    lore: 'Writes small gold orbits in the air and waits for something to step inside.',
    ranged: { cooldownMs: 1900, projectileSpeed: 175, damage: 11 },
    faction: 'River Antler Court',
    role: 'sniper',
    traits: { ghostMs: 420 },
  },
  // Haven of the Bubs -- two factions at war when the player arrives.
  // Bubblenaughts (blue): the map's native population, generations deep.
  {
    id: 'bubblenaught-drifter',
    name: 'Bubblenaught Drifter',
    family: 'Bubblenaught',
    behavior: 'drifter',
    hp: 16,
    speed: 70,
    damage: 5,
    radius: 8,
    xp: 3,
    mass: 0.5,
    palette: { ink: '#03131f', body: '#1d4ed8', bodyDark: '#172554', accent: '#60a5fa', accentBright: '#dbeafe', skin: '#1d4ed8', glow: '#38bdf8' },
    rig: blobRig({ height: 11, width: 11, wings: true }),
    faction: 'Bubblenaught Tide',
    role: 'swarm',
    lore: 'Born in the foam of a bubble realm nobody has mapped all the way through. Floats first, thinks second.',
  },
  {
    id: 'bubblenaught-warden',
    name: 'Bubblenaught Warden',
    family: 'Bubblenaught',
    behavior: 'charger',
    hp: 60,
    speed: 45,
    damage: 12,
    radius: 12,
    xp: 10,
    mass: 2.2,
    palette: { ink: '#03131f', body: '#1e40af', bodyDark: '#172554', accent: '#38bdf8', accentBright: '#e0f2fe', skin: '#1e40af', glow: '#60a5fa' },
    rig: humanoidRig({ height: 20, width: 12, bulk: true, headColor: 'bodyDark' }),
    faction: 'Bubblenaught Tide',
    role: 'anchor',
    lore: 'Stands wherever the kingdom line used to be, generations after anyone remembers where it actually was.',
  },
  {
    id: 'marshal-undertow',
    name: 'Marshal Undertow',
    family: 'Bubblenaught',
    behavior: 'spitter',
    hp: 85,
    speed: 36,
    damage: 9,
    radius: 11,
    xp: 16,
    mass: 1.6,
    palette: { ink: '#02101c', body: '#1e3a8a', bodyDark: '#0c1a3d', accent: '#7dd3fc', accentBright: '#f0f9ff', skin: '#1e3a8a', glow: '#7dd3fc' },
    rig: humanoidRig({ height: 22, width: 11, staff: true, headColor: 'bodyDark' }),
    lore: 'Named for the current that pulls invaders under before they reach the shallows. Answers only to the elder line.',
    ranged: { cooldownMs: 1800, projectileSpeed: 170, damage: 13 },
    faction: 'Bubblenaught Tide',
    role: 'sniper',
  },
  // Bubbleteers (pink): Bulbosa's invading faction, earned through combat.
  {
    id: 'bubbleteer-cadet',
    name: 'Bubbleteer Cadet',
    family: 'Bubbleteer',
    behavior: 'chase',
    hp: 18,
    speed: 75,
    damage: 6,
    radius: 8,
    xp: 3,
    mass: 0.5,
    palette: { ink: '#22091a', body: '#db2777', bodyDark: '#831843', accent: '#f9a8d4', accentBright: '#fff0f7', skin: '#db2777', glow: '#f472b6' },
    rig: blobRig({ height: 11, width: 11, spikes: true }),
    faction: 'Bubbleteer Parade',
    role: 'swarm',
    lore: 'Signed up for Bulbosa\'s crossing without asking what was on the other side. Finding out now.',
  },
  {
    id: 'bubbleteer-shocker',
    name: 'Bubbleteer Shocker',
    family: 'Bubbleteer',
    behavior: 'shockwave',
    hp: 66,
    speed: 32,
    damage: 13,
    radius: 13,
    xp: 12,
    mass: 2.6,
    palette: { ink: '#22091a', body: '#be185d', bodyDark: '#701a45', accent: '#ff7ab8', accentBright: '#ffe4f1', skin: '#be185d', glow: '#ff9ecb' },
    rig: humanoidRig({ height: 19, width: 12, puffs: true }),
    faction: 'Bubbleteer Parade',
    role: 'anchor',
    lore: 'Pops loud enough to rattle the whole shoreline. Learned it from watching Bulbosa do the same, louder.',
  },
  {
    id: 'captain-frothbite',
    name: 'Captain Frothbite',
    family: 'Bubbleteer',
    behavior: 'flanker',
    hp: 95,
    speed: 58,
    damage: 11,
    radius: 10,
    xp: 18,
    mass: 1.3,
    palette: { ink: '#1c0714', body: '#9d174d', bodyDark: '#500724', accent: '#ff9ecb', accentBright: '#fff0f7', skin: '#9d174d', glow: '#ff4fa3' },
    rig: humanoidRig({ height: 23, width: 12, wings: true, headColor: 'bodyDark' }),
    faction: 'Bubbleteer Parade',
    role: 'flanker',
    lore: 'Bulbosa\'s second. Never takes the front line, always takes the credit.',
  },

  /**
   * Oddity-area specials -- see .agents/memory/oddity-arenas.md. Cheap fodder
   * that orbits instead of closing distance, and a rare mass-spawn ghost
   * squad that can't be touched until the reveal. Neither is meant to be
   * farmed for XP; both are built to be dodged.
   */
  {
    id: 'ring-runner',
    name: 'Ring Runner',
    family: 'Oddity',
    behavior: 'ringer',
    hp: 9,
    speed: 96,
    damage: 5,
    radius: 8,
    xp: 2,
    mass: 0.5,
    palette: { ink: '#0a0410', body: '#5b21b6', bodyDark: '#2e1065', accent: '#c4b5fd', accentBright: '#f5f3ff', skin: '#5b21b6', glow: '#c4b5fd' },
    rig: blobRig({ height: 12, width: 10, tendrils: true, wings: true }),
    faction: 'Loop Chorus',
    role: 'swarm',
    sizeClass: 'mini',
    traits: { swayRadius: 150 },
    lore: 'Never learned to walk in a straight line. Never needed to -- a hundred of them in a ring does the job a chase never could.',
  },
  {
    id: 'choir-wraith',
    name: 'Choir Wraith',
    family: 'Oddity',
    behavior: 'wraith',
    hp: 420000,
    speed: 40,
    damage: 8,
    radius: 11,
    xp: 1,
    mass: 1,
    palette: { ink: '#05030a', body: '#312244', bodyDark: '#150f24', accent: '#e9d8ff', accentBright: '#ffffff', skin: '#150f24', glow: '#e9d8ff' },
    rig: humanoidRig({ height: 22, width: 11, hood: true, halo: true, staff: true, headColor: 'bodyDark' }),
    faction: 'Choir of Twenty',
    role: 'sniper',
    ranged: { cooldownMs: 1900, projectileSpeed: 190, damage: 9 },
    traits: { revealMs: 20000, swayRadius: 210, swayMs: 6500 },
    // Not meant to be killed inside a run -- excluded from the Bestiary
    // ratio so it can't cap completion below 100%. See oddity-arenas.md.
    excludeFromBestiary: true,
    lore: 'Twenty voices arrived singing and never stopped. You will not see the first twenty seconds -- only feel the room get colder before the second verse starts.',
  },
  // Neon Arcade -- a shuttered arcade under Old Market Hall, glitching back to life.
  {
    id: 'pixel-wraith',
    name: 'Pixel Wraith',
    family: 'Arcade',
    behavior: 'drifter',
    hp: 15,
    speed: 78,
    damage: 5,
    radius: 7,
    xp: 3,
    mass: 0.4,
    palette: {
      ink: '#0a0016',
      body: '#ff2ec4',
      bodyDark: '#7a1266',
      accent: '#4dfaff',
      accentBright: '#d6fdff',
      skin: '#ff2ec4',
      glow: '#4dfaff',
    },
    rig: blobRig({ height: 11, width: 9, spikes: true }),
    faction: 'Cabinet Rot',
    role: 'swarm',
    traits: { ghostMs: 700, shiftMs: 2200, shiftScale: 1.4 },
    lore: 'A sprite that outlived its own cabinet. Flickers between frames like it is still deciding whether it is real.',
  },
  {
    id: 'token-golem',
    name: 'Token Golem',
    family: 'Arcade',
    behavior: 'charger',
    hp: 70,
    speed: 36,
    damage: 15,
    radius: 15,
    xp: 10,
    mass: 3,
    palette: {
      ink: '#241a04',
      body: '#b8860b',
      bodyDark: '#5c4308',
      accent: '#ffd76a',
      accentBright: '#fff3c4',
      skin: '#8a6a12',
      glow: '#ffd76a',
    },
    rig: humanoidRig({ height: 24, width: 15, bulk: true, torsoColor: 'body' }),
    faction: 'Cabinet Rot',
    role: 'anchor',
    sizeClass: 'elite',
    traits: { burstSpeed: 2 },
    lore: 'Built from decades of dropped quarters fused solid. Still charges like it wants your change back.',
  },
  {
    id: 'high-score-phantom',
    name: 'High Score Phantom',
    family: 'Arcade',
    behavior: 'flanker',
    hp: 26,
    speed: 66,
    damage: 8,
    radius: 8,
    xp: 6,
    mass: 0.6,
    palette: {
      ink: '#020617',
      body: '#38bdf8',
      bodyDark: '#0c4a6e',
      accent: '#e0f2fe',
      accentBright: '#ffffff',
      skin: '#0ea5e9',
      glow: '#7dd3fc',
    },
    rig: humanoidRig({ height: 19, width: 9, halo: true, hood: true, headColor: 'bodyDark' }),
    faction: 'Cabinet Rot',
    role: 'flanker',
    traits: { teleportMs: 3600 },
    lore: 'The initials at the top of the board, still defending a record nobody else remembers setting.',
  },
  {
    id: 'static-swarm',
    name: 'Static Swarm',
    family: 'Arcade',
    behavior: 'ringer',
    hp: 8,
    speed: 100,
    damage: 3,
    radius: 5,
    xp: 2,
    mass: 0.25,
    sizeClass: 'mini',
    palette: {
      ink: '#0a0a0a',
      body: '#9ca3af',
      bodyDark: '#374151',
      accent: '#e5e7eb',
      accentBright: '#ffffff',
      skin: '#6b7280',
      glow: '#d1d5db',
    },
    rig: blobRig({ height: 8, width: 8, tendrils: true }),
    faction: 'Cabinet Rot',
    role: 'swarm',
    traits: { swayRadius: 110 },
    lore: 'Loose signal noise that never found a screen to live on. Circles anything warm enough to be one.',
  },
  {
    id: 'claw-machine-menace',
    name: 'Claw Machine Menace',
    family: 'Boss',
    behavior: 'shockwave',
    hp: 260,
    speed: 28,
    damage: 22,
    radius: 22,
    xp: 32,
    mass: 4.5,
    sizeClass: 'giant',
    palette: {
      ink: '#1a0505',
      body: '#dc2626',
      bodyDark: '#7f1d1d',
      accent: '#fef08a',
      accentBright: '#ffffff',
      skin: '#f8fafc',
      glow: '#facc15',
    },
    rig: arachnidRig({ height: 26, span: 42, legPairs: 2 }),
    faction: 'Cabinet Rot',
    role: 'anchor',
    lore: 'The prize was never in reach. Now it does not need one to close the claw on you.',
  },
  // Neon Overflow -- the arcade's back stock room, one aisle over from Cabinet Rot.
  {
    id: 'marquee-reaper',
    name: 'Marquee Reaper',
    family: 'Boss',
    behavior: 'charger',
    hp: 360,
    speed: 34,
    damage: 26,
    radius: 24,
    xp: 40,
    mass: 5,
    sizeClass: 'giant',
    palette: {
      ink: '#140316',
      body: '#7e22ce',
      bodyDark: '#3b0764',
      accent: '#fde047',
      accentBright: '#fff9c4',
      skin: '#a855f7',
      glow: '#fde047',
    },
    rig: giantRig(32),
    faction: 'High Roller Syndicate',
    role: 'anchor',
    traits: { shiftMs: 2400, shiftScale: 1.3 },
    lore: 'The marquee out front spelled a name nobody could remember choosing. It walks now, still spelling it, one bulb at a time.',
  },
];

export const ENEMIES_BY_ID: Record<string, EnemyDef> = Object.fromEntries(
  ENEMIES.map((e) => [e.id, e]),
);

export function getEnemy(id: string | number): EnemyDef {
  // Numeric coordinates are accepted for legacy test helpers that omit the
  // optional enemy id; authored game data always uses string ids.
  const found = ENEMIES_BY_ID[typeof id === 'number' ? 'nightcrawler' : id];
  if (!found) {
    throw new Error(`Unknown enemy id: ${id}`);
  }
  return found;
}
