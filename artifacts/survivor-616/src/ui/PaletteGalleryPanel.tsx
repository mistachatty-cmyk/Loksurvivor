    ...
    <ScreenLayout title="Customization Shop" subtitle={`${vendorName} — Paint Gallery`} onBack={onBack}>
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
        <aside className="flex shrink-0 flex-col items-center gap-3 border border-border bg-card px-6 py-5 lg:sticky lg:top-4 lg:w-72">
          ...
          <div className="w-full"><ShopTabs active={category} onChange={(nextCategory) => { setCategory(nextCategory); setVisibleCount(8); }} /></div>
          <div className="grid w-full grid-cols-2 gap-1">
            <button type="button" onClick={() => setWorldPaletteBlend(!meta.worldPaletteBlendEnabled)} aria-pressed={meta.worldPaletteBlendEnabled} className="border border-border px-2 py-2 font-mono text-[8px] uppercase text-white/70">Blend {meta.worldPaletteBlendEnabled ? 'on' : 'off'}</button>
            <button type="button" onClick={() => setPaletteAnimations(!meta.paletteAnimationsEnabled)} aria-pressed={meta.paletteAnimationsEnabled} className="border border-border px-2 py-2 font-mono text-[8px] uppercase text-white/70">Motion {meta.paletteAnimationsEnabled ? 'on' : 'off'}</button>
          </div>
        ...
        <section className="flex-1 border border-border bg-card p-5 sm:p-6" data-testid="section-customization-shop">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              {category === 'palettes' ? (
                <div className="mt-5 grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-3" data-testid="shop-palette-grid">
                    ...
                    return (
                      <article key={palette.id} className={`border p-2.5 sm:p-4 ${equipped ? 'border-primary bg-primary/5' : 'border-border bg-background'}`} data-testid={`card-palette-${palette.id}`}>
                        ...
                        <div className="mt-2 flex gap-1" role="img" aria-label={`${palette.name} color preview`}>
                          {[palette.palette.body, palette.palette.accent, palette.palette.accentBright, palette.palette.glow].map((color, colorIndex) => <span key={color} className={`h-5 flex-1 border border-white/10 ${palette.effect && meta.paletteAnimationsEnabled ? EFFECT_PREVIEW_CLASSES[palette.effect.kind] : ''}`} style={{ backgroundColor: color, animationDelay: `${colorIndex * 90}ms` }} />)}
                        </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-3" data-testid="shop-aura-grid">
                  {RUN_AURAS.slice(0, visibleCount).map((aura) => {
                  ...
                  {(category === 'hats' ? HATS : CELEBRATIONS).slice(0, visibleCount).map((item) => {
                    ...
                    return (
                      <article key={item.id} className={`border p-4 ${equipped ? 'border-primary bg-primary/5' : 'border-border bg-background'}`}>
                        ...
                        <button type="button" onClick={preview} className="mt-3 w-full border border-white/20 px-3 py-1.5 font-mono text-[8px] uppercase tracking-widest text-white/80">{isHat ? 'Preview on Valor' : 'Play celebration'}</button>
