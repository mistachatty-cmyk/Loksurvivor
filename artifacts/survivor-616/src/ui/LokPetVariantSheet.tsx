...
import type { LokPetSilhouette } from '@/game/types';

const SHAPE_CLIPS: Record<LokPetSilhouette, string> = {
        ...
        {LOKPET_VARIANTS.map((variant) => (
          <div key={variant.id} className="flex min-w-0 items-center gap-2 border border-white/10 bg-black/35 p-2">
            <div
              ...
              style={{
                backgroundColor: variant.palette.body,
                borderColor: variant.palette.accent,
                clipPath: SHAPE_CLIPS[variant.silhouette],
                boxShadow: `0 0 12px ${variant.palette.glow}66`,
              }}
              aria-hidden="true"
            />
