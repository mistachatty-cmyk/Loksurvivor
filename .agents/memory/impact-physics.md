...
Clickable movable props are a one-shot mechanic: pointer selection primes the next player impact, which launches the prop **the way the hit sent it** -- away from the impact point -- with a **rolled** velocity multiplier (5-9x) and a floor speed so even a light tap on a heavy prop visibly flies. Reversing that launch back toward whoever swung is the opt-in "Backspin Rig" mode (`World.physicsObjectReverseLaunch`, from a Quartermaster purchase plus a Settings toggle), **off by default**, and it rolls from a deliberately higher band (10-18x, with a much higher floor speed) because reversing a hit fights the prop's own momentum -- at the ordinary multiplier a backspun prop barely moves. Sweep the prop from its previous to current position when applying path damage.
...
**How to apply:**  
Keep priming gated by the persisted physics-interaction setting, consume the prime on the next qualifying player impact, and preserve the existing idempotent enemy-kill path for launch damage. Sealed props (`ObstacleDef.sealed`, or a per-run roll on volatile kinds from the separate `propRng` stream) take **no** damage at all until a tap/click cracks the seal -- the same click that primes an ordinary prop. That is what makes a one-hit prop like a gas tank an encounter rather than a trip hazard: armour, a deliberate click, then one hit. Prop layout rolls (seals, prefab scatter) must stay on `propRng`, never `rng`: scenery changes that shift the combat/loot stream break seeded runs.

A prop in flight **shoves the player and never damages them** (`shovePlayerFromMovingProp`, force into `player.kx/ky`): your own launched scenery is a mobility tool, so hitting yourself with it is a reposition, not a punishment.
