...
- [Kinetic Bender, time abilities and the pause read model](time-abilities-and-pause.md) — a freeze is a gate on which subsystems step while run speed is one composed clamp; kits live in their own room and reuse the prop-impact path; the pause menu reads existing timers instead of keeping its own.
